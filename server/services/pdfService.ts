...
export class PDFService {
  ...
  async generatePDF(html: string, filename: string): Promise<string> {
    ...
    try {
      ...
      const isCompleteHtml = html.trim().toLowerCase().startsWith('<!doctype') || 
      ...
      if (isCompleteHtml) {
        ...
        console.log('📄 Using complete HTML document for PDF');
        finalHtml = html;
      } else {
        // HTML is a fragment, wrap it with styling
        console.log('📄 Wrapping HTML fragment for PDF');
        finalHtml = `
          <!DOCTYPE html>
          <html lang="nl">
          <head>
